...
if (isset($_GET['unblock_id'])) {
    ...
    $stmt = mysqli_prepare($conn, "UPDATE users SET is_blocked = 0, failed_attempts = 0, account_status = 'active' WHERE id = ?");
    if ($stmt) {
...
<section class="hero-banner">
    <div class="hero-text">
        ...
        <p>Deactivate accounts to block login access, or activate them again when needed.</p>
    </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <form method="POST" style="margin:0;">
                                ...
                                <div style="display:flex;align-items:center;gap:8px;min-width:220px;">
                                    <select
                                        name="new_role"
                                        style="flex:1;height:42px;padding:0 12px;border:1px solid #dbe7f0;border-radius:12px;background:#ffffff;font-size:13px;color:#0f172a;outline:none;"
                                    >
                                        ...
                                        <option value="itsupport" <?php echo $user['role'] === 'itsupport' ? 'selected' : ''; ?>>IT Support</option>
                                    </select>

                                    <button
                                        type="submit"
                                        name="update_role"
                                        style="height:42px;padding:0 14px;border:none;border-radius:12px;background:linear-gradient(90deg,#0ea5a4,#14b8a6);color:white;font-size:12px;font-weight:700;cursor:pointer;white-space:nowrap;"
                                    >
                                        Save
                                    </button>
                                </div>
                        ...
                        <td>
                            <div class="manage-user-actions">
                                ...
                                <?php if ((int) $user['is_blocked'] === 1 && $user['account_status'] !== 'inactive') { ?>
                                    <a href="manageusers.php?unblock_id=<?php echo $user['id']; ?>" class="action-btn approve">Unblock</a>
                                <?php } ?>

                                <?php if ($user['account_status'] === 'active') { ?>
                                    <a href="manageusers.php?toggle_status=<?php echo $user['id']; ?>" class="action-btn reject" onclick="return confirm('Deactivate this account and block login access?');">Deactivate</a>
                                <?php } elseif ($user['account_status'] === 'inactive') { ?>
                                    <a href="manageusers.php?toggle_status=<?php echo $user['id']; ?>" class="action-btn approve" onclick="return confirm('Activate this account again?');">Activate</a>
