...
$totalAdmins = getSingleValue($conn, "SELECT COUNT(*) AS total FROM users WHERE role = 'admin'");
$totalHR = getSingleValue($conn, "SELECT COUNT(*) AS total FROM users WHERE role = 'hr'");
$totalEmployees = getSingleValue($conn, "SELECT COUNT(*) AS total FROM users WHERE role = 'employee'");
$totalTeamLeaders = getSingleValue($conn, "SELECT COUNT(*) AS total FROM users WHERE role = 'teamleader'");
$totalPendingSetup = getSingleValue($conn, "SELECT COUNT(*) AS total FROM users WHERE account_status = 'pending_setup'");
$totalActiveUsers = getSingleValue($conn, "SELECT COUNT(*) AS total FROM users WHERE account_status = 'active'");

$totalPendingRequests = 0;
if (tableExists($conn, "requests")) {
    $totalPendingRequests = getSingleValue($conn, "SELECT COUNT(*) AS total FROM requests WHERE status = 'pending'");
}

$newHiresThisMonth = 0;
if (columnExists($conn, "users", "created_at")) {
    $newHiresThisMonth = getSingleValue($conn, "
...
$activeUsersRate = 0;
if ($totalUsers > 0) {
    $activeUsersRate = round(($totalActiveUsers / $totalUsers) * 100);
}

$totalTickets = 0;
$completedTickets = 0;

if (tableExists($conn, "team_tickets")) {
    $totalTickets = getSingleValue($conn, "SELECT COUNT(*) AS total FROM team_tickets");
    $completedTickets = getSingleValue($conn, "SELECT COUNT(*) AS total FROM team_tickets WHERE status = 'completed'");
}

$taskCompletionRate = 0;
if ($totalTickets > 0) {
    $taskCompletionRate = round(($completedTickets / $totalTickets) * 100);
}
...
if (tableExists($conn, "leave_requests")) {
    $totalLeaveRequests = getSingleValue($conn, "SELECT COUNT(*) AS total FROM leave_requests");
    $approvedLeaveRequests = getSingleValue($conn, "SELECT COUNT(*) AS total FROM leave_requests WHERE status = 'Approved'");
}

$leaveApprovalRate = 0;
if ($totalLeaveRequests > 0) {
    $leaveApprovalRate = round(($approvedLeaveRequests / $totalLeaveRequests) * 100);
}

$openSupportTickets = 0;
if (tableExists($conn, "support_tickets")) {
    $openSupportTickets = getSingleValue($conn, "
        SELECT COUNT(*) AS total
...
$latestKpiUpdate = date("Y-m-d H:i:s");

$kpis = [
    [
        "metric_title" => "New Hires This Month",
        "metric_value" => $newHiresThisMonth,
        "metric_description" => "Employees added during the current month",
        "status" => $newHiresThisMonth > 0 ? "good" : "normal",
        "updated_at" => $latestKpiUpdate
    ],
    [
        "metric_title" => "Active Users Rate",
        "metric_value" => $activeUsersRate . "%",
        "metric_description" => "Active accounts compared to all system users",
        "status" => $activeUsersRate >= 70 ? "good" : "warning",
        "updated_at" => $latestKpiUpdate
    ],
    [
        "metric_title" => "Task Completion Rate",
        "metric_value" => $taskCompletionRate . "%",
        "metric_description" => "Completed team tickets compared to all assigned tickets",
        "status" => $taskCompletionRate >= 70 ? "good" : "warning",
        "updated_at" => $latestKpiUpdate
    ],
    [
        "metric_title" => "Leave Approval Rate",
        "metric_value" => $leaveApprovalRate . "%",
        "metric_description" => "Approved leave requests compared to all submitted leave requests",
        "status" => $leaveApprovalRate >= 60 ? "good" : "normal",
        "updated_at" => $latestKpiUpdate
    ],
    [
        "metric_title" => "Pending Access Requests",
        "metric_value" => $totalPendingRequests,
        "metric_description" => "Access requests still waiting for admin review",
        "status" => $totalPendingRequests > 0 ? "warning" : "good",
        "updated_at" => $latestKpiUpdate
    ],
    [
        "metric_title" => "Open Support Tickets",
        "metric_value" => $openSupportTickets,
        "metric_description" => "Support tickets that are still not closed",
        "status" => $openSupportTickets > 0 ? "warning" : "good",
        "updated_at" => $latestKpiUpdate
    ]
];
...
<aside class="sidebar">
    ...
    <div class="sidebar-bottom">
        <div class="system-card">
            ...
            <h4>Live Data</h4>
            <span>Calculated from database</span>
        </div>
    </div>
</aside>
...
<section class="analytics-hero">
    <div class="analytics-hero-top">
        <div>
            ...
            <p>
                This page displays real performance indicators calculated from the database.
                It helps the admin understand user activity, requests, task progress, and system operations.
            </p>
        </div>
    ...
    <div class="panel">
        ...
        <div class="insight-board">
            <div class="insight-card">
                ...
                <p>These accounts still need to complete their initial setup before becoming fully active.</p>
            </div>

            <div class="insight-card">
                <i class="fas fa-file-circle-question"></i>
                <h4><?php echo $totalPendingRequests; ?> Pending Requests</h4>
                <p>These access requests are still waiting for admin review and action.</p>
            </div>

            <div class="insight-card">
                <i class="fas fa-user-tie"></i>
                <h4><?php echo $totalHR; ?> HR Accounts</h4>
                <p>This is the current number of HR members available for employee operations.</p>
            </div>

            <div class="insight-card">
                <i class="fas fa-users"></i>
                <h4><?php echo $totalUsers; ?> Total Accounts</h4>
                <p>This is the total number of accounts across all system roles.</p>
            </div>
        </div>
    </div>
