    ...
    if (in_array($newRole, $allowedRoles, true)) {
        ...
        if ($stmt) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                $popupMessage = "HR member role updated successfully.";
                $popupType = "success";
            } else {
                $popupMessage = "Failed to update role.";
                $popupType = "error";
            }
...
if (isset($_GET['unblock_id'])) {
    ...
    if ($stmt) {
        ...
        mysqli_stmt_close($stmt);
        $popupMessage = "HR account unblocked successfully.";
        $popupType = "success";
    }
}
...
$hrCount = 0;
$hrCountQuery = "SELECT COUNT(*) AS total FROM users WHERE role = 'hr'";
$hrCountResult = mysqli_query($conn, $hrCountQuery);
if ($hrCountResult && $row = mysqli_fetch_assoc($hrCountResult)) {
    $hrCount = $row['total'];
}

$activeHrCount = 0;
$activeHrQuery = "SELECT COUNT(*) AS total FROM users WHERE role = 'hr' AND account_status = 'active'";
$activeHrResult = mysqli_query($conn, $activeHrQuery);
if ($activeHrResult && $row = mysqli_fetch_assoc($activeHrResult)) {
    $activeHrCount = $row['total'];
}

$pendingHrCount = 0;
$pendingHrQuery = "SELECT COUNT(*) AS total FROM users WHERE role = 'hr' AND account_status = 'pending_setup'";
$pendingHrResult = mysqli_query($conn, $pendingHrQuery);
if ($pendingHrResult && $row = mysqli_fetch_assoc($pendingHrResult)) {
    $pendingHrCount = $row['total'];
}

$blockedHrCount = 0;
$blockedHrQuery = "SELECT COUNT(*) AS total FROM users WHERE role = 'hr' AND is_blocked = 1";
$blockedHrResult = mysqli_query($conn, $blockedHrQuery);
if ($blockedHrResult && $row = mysqli_fetch_assoc($blockedHrResult)) {
    $blockedHrCount = $row['total'];
}
    ...
    <aside class="sidebar">
      ...
      <div class="sidebar-bottom">
        <div class="system-card">
          <p>HR Overview</p>
          <h4><?php echo $hrCount; ?> Members</h4>
          <span><?php echo $activeHrCount; ?> active accounts</span>
        </div>
      </div>
    </aside>
    ...
    <main class="main-content">
      ...
      <header class="topbar">
        <div class="topbar-left">
          <h1>HR Team</h1>
          <p>Review HR members, their account status, and update their roles when needed.</p>
        </div>
      ...
      <section class="hero-banner">
        <div class="hero-text">
          <h2>HR team overview 👥</h2>
          <p>Monitor HR members, track pending setup accounts, and manage role changes from one place.</p>
        </div>
        <div class="hero-actions">
          <a href="manageusers.php" class="hero-btn primary-btn">
      ...
      <section class="cards">
        <div class="card">
          ...
          <div class="card-info">
            ...
            <span>All accounts with HR role</span>
          </div>
        </div>
        ...
        <div class="card">
          ...
          <div class="card-info">
            <h3><?php echo $activeHrCount; ?></h3>
            <p>Active HR Accounts</p>
            <span>Ready to use the system</span>
          </div>
        </div>
        ...
        <div class="card">
          ...
          <div class="card-info">
            <h3><?php echo $pendingHrCount; ?></h3>
            <p>Pending Setup</p>
            <span>Still need to complete setup</span>
          </div>
        </div>
        ...
        <div class="card">
          ...
          <div class="card-info">
            <h3><?php echo $blockedHrCount; ?></h3>
            <p>Blocked HR Accounts</p>
        ...
        <div class="table-wrapper">
          <table>
            ...
            <tbody>
                  ...
                  <tr>
                    ...
                    <td>
                      <div class="manage-user-actions">
                        ...
                        <?php if ((int) $user['is_blocked'] === 1) { ?>
                          <a href="hrteam.php?unblock_id=<?php echo $user['id']; ?>" class="action-btn approve">Unblock</a>
                        <?php } ?>
                      </div>
                    </td>
                  </tr>
              ...
              <?php } else { ?>
                <tr>
                  <td colspan="7">No HR members found.</td>
                </tr>
              <?php } ?>
            </tbody>
          </table>
